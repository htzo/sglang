...
at::Tensor causal_conv1d_update_cpu(
  ...
  // Multi-token (target verify) path: process each time step sequentially,
  // updating conv_state after each step. `x` may be non-contiguous (the GDN
  // verify call site passes a [batch, dim, seqlen] transpose view of a
  // token-major buffer); per-step slices are gathered as needed. The output is
  // allocated token-major ([batch, seqlen, dim] storage) and returned as its
  // [batch, dim, seqlen] transpose so the caller's `.transpose(1, 2).view(...)`
  // stays a zero-copy view.
  TORCH_CHECK(
  ...
  for (int64_t t = 0; t < seqlen; ++t) {
    ...
    AT_DISPATCH_REDUCED_FLOATING_TYPES(scalar_type, "causal_conv1d_update_kernel_impl", [&] {
      ...
      // Cache intermediate conv states for the post-verify rollback.
      // conv_states has strides {state_len*dim, 1, dim} after as_strided_
      // (physical: [state_len, dim] per entry, dim contiguous).
      // intermediate_conv_window is [cache_size, steps, dim, state_len]
      // contiguous (physical: [dim, state_len] per entry, state_len
      // contiguous). We must transpose during the copy.
      if (cache_intermediate) {
        ...
        const int64_t icw_seq_stride = icw_steps * icw_entry_size;
